...
<body> 
	...
	<form id="form1" method="post">
			...
			<table style="table-layout:fixed;">
				<tr>
					...
					<td style="width:150px;">
						<input class="mini-buttonedit"  required="true"  onbuttonclick="onSelectDealer" id="dealer_no" name="dealer_no"  value="<?php echo $show['dealer_no'];?>" text="<?=$dealer['dealer_name']?>" allowInput="false"/>
					</td>
					<td style="width:80px;">所属客户：</td>
					<td style="width:150px;">
						<input class="mini-buttonedit" required="true"  onbuttonclick="onSelectCust" id="cust_no" name="cust_no" value="<?php echo $show['cust_no'];?>" text="<?=$cust['cust_name']?>" allowInput="false"/>
					</td>
				...
				<tr>
					...
					<td style="width:150px;">
						<select id="payment_type" name="payment_type" class="mini-combobox" style="width:150px;">
							<option value="0" <?php if($show['payment_type'] == 0){echo 'selected';} ?>>现金</option>
							<option value="1" <?php if($show['payment_type'] == 1){echo 'selected';} ?>>月结</option>
						</select>
					</td>
		...
		<div class="mini-fit" style="background:white;height:300px;">
			<div id="datagrid1" class="mini-datagrid" style="width:100%;height:300px" url="index.php?m=system&c=outhouseOrderNum&a=getItemList&order_no=<?=$show['order_no']?>" 
			multiSelect="true" allowCellEdit="true" allowAlternating="true" allowCellSelect="true" idField="id" showPager="false"
			...
			editNextRowCell="true">
				<div property="columns">
					...
					<div type="checkboxcolumn" trueValue="1" falseValue="0" field="is_gift" width="70" headerAlign="center" align="center" allowSort="" align="center">是否赠品</div>
	...
	<script>
        ...
        function SaveData() {
            ...
            $.ajax({
                url: "index.php?m=system&c=outhouseOrderNum&a=edit",
		        type: 'post',
		...
		function delRow111(row_uid) {
			...
			if (row.id) {
				if (confirm("确定删除此记录？")) {
					grid.loading("删除中，请稍后......");
					$.ajax({
						url: "index.php?m=system&c=outhouseOrderNum&a=delItem&id=" + row.id,
