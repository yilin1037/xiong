...
<html>
	...
	<div class="mini-fit">
        ...
        url="index.php?m=system&c=user&a=baseConfigList" idField="id" emptyText="数据为空，<a href='javascript:newRow()'>增加一条</a>" showEmptyText="true"
    >
			<div property="columns">
				<div name="action" width="80" headerAlign="center" align="center" renderer="onActionRenderer" cellStyle="padding:0;">#</div>
				
				<div field="configKey" width="120" headerAlign="center" allowSort="">编号
					<input property="editor" class="mini-textbox" style="width:100%;"/>
				</div>
				<div field="configValue" width="120" headerAlign="center" allowSort="">名称
					<input property="editor" class="mini-textbox" style="width:100%;"/>
				</div>
			</div>
		</div>    
		...
		<script type="text/javascript">
			...
			function delRow(row_uid) {
				...
				if (row) {
					if (confirm("确定删除此记录？")) {
						...
						$.ajax({
							url: "index.php?m=system&c=user&a=delBaseConfig&id=" + row.id,
			...
			function updateRow(row_uid) {
				...
				$.ajax({
					url: "index.php?m=system&c=user&a=saveBaseConfig",
					type: 'post',
					dataType:'json',
					cache: false,
					data: { data: json },
