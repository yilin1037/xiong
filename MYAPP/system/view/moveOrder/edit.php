...
<body> 
			...
			<table style="table-layout:fixed;">
				<tr>
					...
					<td style="width:150px;">
						<input class="mini-buttonedit"  required="true"  onbuttonclick="onSelectDealer" id="dealer_no" name="dealer_no"  value="<?php echo $show['dealer_no'];?>" text="<?=$dealer['dealer_name']?>"/>
					</td>
				</tr>
				<tr>
	...
	<script>
        ...
        function SaveData() {
			...
			if(!dataItem[0]['prd_no']){
				
				alert('商品编号不可为空');
				return;
			}
			
			if(!dataItem[0]['add_unit_num']){
				
				alert('数量不可为空');
				return;
			}
            $.ajax({
                url: "index.php?m=system&c=moveOrder&a=edit",
		        type: 'post',
