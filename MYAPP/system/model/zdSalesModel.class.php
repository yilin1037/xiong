<?
class zdSalesModelClass extends CommonModelClass
{
			...
			foreach($data2 as $vo){
				...
				if(is_array($vo['ZH_detail']) && !empty($vo['ZH_detail'])){
					foreach($vo['ZH_detail'] as $vo1){
						if(!empty($vo1['money'])){
							$sql = "insert into ". TABLE('sales_promotion_sub') ."(sales_no,group_name,money,level) values (";
							$sql .= " '". $sales_no ."', ";
							$sql .= " '". $vo['groupId'] ."', ";
							$sql .= " '". $vo1['money'] ."', ";
							$sql .= " '". $vo['orderId'] ."' ";
	...
	function saveEdit($params, $data2){
		...
		if(!empty($data2)){
			...
			foreach($data2 as $vo){
				if(is_array($vo['ZH_detail']) && !empty($vo['ZH_detail'])){
					foreach($vo['ZH_detail'] as $vo1){
						if(empty($vo1['id']) && !empty($vo1['money'])){
							$sql = "insert into ". TABLE('sales_promotion_sub') ."(sales_no,group_name,money,level) values (";
							$sql .= " '". $sales_no ."', ";
							$sql .= " '". $vo['groupId'] ."', ";
							$sql .= " '". $vo1['money'] ."', ";
						...
						if(intval($vo1['id']) > 0 && !empty($vo1['money'])){
							$sql = "UPDATE ". TABLE('sales_promotion_sub');
							$sql .= " SET group_name = '". $vo['groupId'] ."', ";
							$sql .= " money = '". $vo1['money'] ."', ";
							$sql .= " level = '". $vo['orderId'] ."' ";
							$sql .= " WHERE id=". $vo1['id'];
							$rs = $model->execute($sql);
							//echo $model->getLastSql();exit;
							if($rs === false){
				...
				if(is_array($vo['ZP_detail']) && !empty($vo['ZP_detail'])){
					foreach($vo['ZP_detail'] as $vo2){
						if(empty($vo2['id']) && !empty($vo2['prd_no'])){
							$sql = "insert into ". TABLE('sales_promotion_gift') ."(sales_no,group_name,prd_no,unit_qty) values (";
							$sql .= " '". $sales_no ."', ";
							$sql .= " '". $vo['groupId'] ."', ";
							$sql .= " '". $vo2['prd_no'] ."', ";
						...
						if(intval($vo2['id']) > 0 && !empty($vo2['prd_no'])){
							$sql = "UPDATE ". TABLE('sales_promotion_gift');
							$sql .= " SET group_name = '". $vo['groupId'] ."', ";
							$sql .= " prd_no = '". $vo2['prd_no'] ."', ";
							$sql .= " unit_qty = '". $vo2['unit_qty'] ."' ";
							$sql .= " WHERE id=". $vo2['id'];
							$rs = $model->execute($sql);
							if($rs === false){
								$model->rollback();
	...
	function getItemList($params){
		...
		$sql = "select id,sales_no,group_name,money,level from " . TABLE('sales_promotion_sub') . " where sales_no=:sales_no ORDER BY id DESC";
		...
		if($rows = $model->query($sql, $sqlParam)->select())
		{
			foreach($rows as $row)
			{
				...
				$data[$row['group_name']]['ZH_detail'][] = array('id' => $row['id'],'money' => $row['money'],'sales_no' => $row['sales_no'],'group_name' => $row['group_name']);
			}
		}
	...
	function del($id){
		...
		$where = " WHERE 1=1 AND sales_type='zdmz' ";
		if(strpos($id, ',') !== false){
			$id = explode(',', $id);
			$id = implode(',', $id);
			$where .= ' AND id in('.$id.') ';
	...
	/**
	 * 删除 sales_promotion_gift 和 sales_promotion_sub 表 单条记录
	 */
	/* function delItem($ids){
		...
		$sql = "DELETE FROM " . TABLE('sales_promotion_gift') . $where;
		$sql = "DELETE FROM " . TABLE('sales_promotion_sub') . $where;
