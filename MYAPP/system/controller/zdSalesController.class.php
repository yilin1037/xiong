<?
class zdSalesControllerClass extends Controller{
	public $model;
	public function __construct(){
		parent::__construct();
		$this->model = M('zdSales');
	}
    public function index(){
        $this->userActionLimitCheck(array(110402));
		$this->display();
    }
	 ...
	 */
	function add(){
        $this->userActionLimitCheck(array(110401));
	 ...
	 */
	function edit(){
        $this->userActionLimitCheck(array(110403));
		if($_SERVER['REQUEST_METHOD'] == 'POST'){
			$data	= $_POST['data'];
			$data 	= json_decode($data, true);
			$data2 	= $_POST['data2'];
	 ...
	 */
	function del(){
        $this->userActionLimitCheck(array(110404));
